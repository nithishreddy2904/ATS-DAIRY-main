...
const PURPLE = "#7b1fa2"; // Same as ProcessingUnits maintenance tab
...
const MaintenanceTable = () => {
  ...
  useEffect(() => {
    ...
    socket.on('maintenance:created', payload => {
      setRecords(prev => [payload.data, ...prev]);
    });
    socket.on('maintenance:updated', payload => {
      setRecords(prev =>
        prev.map(r => (r.id === payload.data.id ? payload.data : r))
      );
    });
    socket.on('maintenance:deleted', payload => {
  ...
  const handleEditSave = async updatedRecord => {
    try {
      await maintenanceService.updateRecord(updatedRecord.id, updatedRecord);
      await loadRecords();
  ...
  const handleDeleteConfirm = async () => {
    try {
      await maintenanceService.deleteRecord(selectedRecord.id);
      await loadRecords();
  ...
  return (
    <Box p={2}>
      ...
      <Box sx={{ display: "flex", alignItems: "center", mb: 2 }}>
        <Build sx={{ color: PURPLE, fontSize: 32, mr: 1 }} />
